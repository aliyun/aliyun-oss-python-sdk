...
def parse_lifecycle_version_expiration(version_expiration_node):
    ...
    non_current_days = _find_int(version_expiration_node, 'NoncurrentDays')
    expiration = NonCurrentVersionExpiration(non_current_days)
...
def parse_get_bucket_lifecycle(result, body):
    ...
    for rule_node in root.findall('Rule'):
        ...
        non_current_version_expiration = parse_lifecycle_version_expiration(rule_node.find('NoncurrentVersionExpiration'))
        non_current_version_sotrage_transitions = parse_lifecycle_verison_storage_transitions(rule_node.findall('NoncurrentVersionTransition'))
        ...
        rule = LifecycleRule(
            ...
            non_current_version_sotrage_transitions = non_current_version_sotrage_transitions
            )
        result.rules.append(rule)
...
def to_put_bucket_lifecycle(bucket_lifecycle):
    ...
    for rule in bucket_lifecycle.rules:
        ...
        non_current_version_expiration = rule.non_current_version_expiration
        if non_current_version_expiration is not None:
            ...
            _add_text_child(version_expiration_node, 'NoncurrentDays', str(non_current_version_expiration.non_current_days))

        non_current_version_sotrage_transitions = rule.non_current_version_sotrage_transitions
        if non_current_version_sotrage_transitions is not None:
            for non_current_version_sotrage_transition in non_current_version_sotrage_transitions:
                version_transition_node = ElementTree.SubElement(rule_node, 'NoncurrentVersionTransition')
                _add_text_child(version_transition_node, 'NoncurrentDays', str(non_current_version_sotrage_transition.non_current_days))
                _add_text_child(version_transition_node, 'StorageClass', str(non_current_version_sotrage_transition.storage_class))
