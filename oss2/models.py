...
class RetrievedReplicationRule(ReplicationRule):
    """获取到到Bucket跨区域复制规则
    ...
    :param list action_list: 新写入的数据与历史数据需要遵循的Action类型, 其中"ALL" 表示"PUT", "ABORT"与"DELETE"操作。
            均会被同步到Bucket，"PUT"表示同步到目标Bucket的写入操作。
    :param bool is_enable_historical_object_replication: 指定是否复制历史数据。
    :param str sync_role_name: 授权OSS使用哪个角色来进行数据复制。如果指定使用SSE-KMS加密目标对象，则必须指定sync_role_name。
    :param str replica_kms_keyid: 指定SSE-KMS密钥ID
    :param str sse_kms_encrypted_objects_status: 指定OSS是否复制通过SSE-KMS加密创建的对象。
            取值范围[ReplicationRule.ENABLED, ReplicationRule.DISABLED]。
    :param str status: 复制状态，可能为:starting, doing 或closing。
    """
    ...
    def __init__(self,
                 ...
                 sse_kms_encrypted_objects_status=None,
                 status=None):
        super(RetrievedReplicationRule, self).__init__(rule_id, target_bucket_name, target_bucket_location,
                target_transfer_type, prefix_list, action_list, is_enable_historical_object_replication,
                sync_role_name, replica_kms_keyid, sse_kms_encrypted_objects_status)
        self.status = status
...
class GetBucketReplicationResult(RequestResult):
    ...
    :type rule_list:  list， 元素类型为class:`RetrievedReplicationRule <oss2.models.RetrievedReplicationRule>`。
    """
    def __init__(self, resp):
