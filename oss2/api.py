...
class Bucket(_Base):
    ...
    def put_bucket_replication(self, replication_configuration):
        """设置bucket跨区域复制规则

        :param replication_configuration :class:`ReplicationRule <oss2.models.ReplicationRule>`
        :return: :class:`RequestResult <oss2.models.RequestResult>`
        """
        logger.debug("Start to put bucket replication: {0}".format(self.bucket_name))
        data = xml_utils.to_put_bucket_replication(replication_configuration)
        resp = self.__do_bucket('POST', data=data, params={Bucket.REPLICATION: '', 'comp': 'add'})
    ...
    def delete_bucket_replication(self, rule_id):
        ...
        resp = self.__do_bucket('POST', data=data, params={Bucket.REPLICATION: '', 'comp': 'delete'})
