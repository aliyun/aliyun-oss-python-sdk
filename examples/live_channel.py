...
# 对创建好的频道，可以使用list_live_channel来进行列举已达到管理的目的。
# list_live_channel可以指定prefix、marker和max_keys这三个参数。
# prefix可以按照前缀过滤list出来的频道。
# marker制定了这次list开始的标记位置。
# max_keys表示一次list出来的频道的最大数量，这个值最大不能超过1000，不填写的话默认为100。
...
while True:
    list_result = bucket.list_live_channel(prefix = prefix, marker = marker, max_keys = max_keys)
    marker = list_result.next_marker
    print len(list_result.channels)
