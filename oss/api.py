...
对象上传方法中的data参数
----------------------
诸如 :func:`put_object <Bucket.put_object>` 这样的上传接口都会有 `data` 参数用户接收用户数据。`data` 可以是下述类型
...
返回值
------
主要包含 :class:`Service` 和 :class:`Bucket` 类，它们的大多数方法都是返回 :class:`RequestResult <oss.models.RequestResult>`
...
class Bucket(_Base):
    ...
    def get_object(self, object_name, range=None, headers=None):
        ...
        :param object_name: 对象名
        :param range: None表示下载整个对象；若是tuple，则是分段下载（Range Get）
