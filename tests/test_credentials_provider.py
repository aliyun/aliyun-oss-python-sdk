...
class TestCredentialsProvider(OssTestCase):
    ...
    def tearDown(self):
        ...
        service = oss2.Service(provider_auth, self.endpoint)
        buckets = service.list_buckets(prefix=OSS_BUCKET).buckets
        for b in buckets:
            bucket = oss2.Bucket(provider_auth, b.extranet_endpoint, b.name)
    ...
    def test_auth(self):
        ...
        access_secret = credentials.get_secret_access_key()
