...
class TestObject(OssTestCase):
    ...
    def test_process_object(self):
        ...
        process = "image/resize,w_100|sys/saveas,o_{0},b_{1}".format(base64.urlsafe_b64encode(dest_key),
                                                                     base64.urlsafe_b64encode(self.bucket.bucket_name))
        ...
        dest_key = self.random_key(".jpg")
        process = "image/resize,w_100|sys/saveas,o_{0}".format(base64.urlsafe_b64encode(dest_key))
        result = self.bucket.process_object(key, process)
        self.assertEqual(result.status, 200)
        result = self.bucket.object_exists(dest_key)
        self.assertEqual(result, True)
