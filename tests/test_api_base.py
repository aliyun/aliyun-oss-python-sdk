...
class TestApiBase(OssTestCase):
    if OSS_CNAME:
        def test_cname_bucket(self):
            bucket = oss2.Bucket(oss2.Auth(OSS_ID, OSS_SECRET), OSS_CNAME, self.OSS_BUCKET, is_cname=True, region=OSS_REGION)
    ...
    def test_https(self):
        ...
        bucket = oss2.Bucket(oss2.AnonymousAuth(), OSS_ENDPOINT.replace('http://', 'https://'), bucket_name, region=OSS_REGION)
    ...
    def test_ip(self):
        ...
        bucket = oss2.Bucket(oss2.AnonymousAuth(), ip, bucket_name, region=OSS_REGION)
        self.assertRaises(oss2.exceptions.NoSuchBucket, bucket.get_object, 'hello.txt')
