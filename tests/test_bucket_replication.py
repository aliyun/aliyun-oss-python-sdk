...
import oss2
from .common import *
from oss2.models import ReplicationRule, RetrievedReplicationRule
...
class TestBucketReplication(OssTestCase):
    ...
    def test_replication_with_full_parameter(self):
        ...
        self.assertEqual(RetrievedReplicationRule.STARTING, rule.status)
        ...
        self.assertEqual(RetrievedReplicationRule.STARTING, progress.status)
        ...
        result = self.source_bucket.get_bucket_replication()
        self.assertEqual(RetrievedReplicationRule.CLOSING, result.rule_list[0].status)

        result = self.source_bucket.get_bucket_replication_progress(rule_id)
        self.assertEqual(RetrievedReplicationRule.CLOSING, result.progress.status)
